...
#
u_var <- indata_ts[, 2]  # Assuming the first column is the target variable
lagged_u <- do.call(cbind, lapply(1:lags, function(k) stats::lag(u_var, -k)))
colnames(lagged_u) <- c(paste0("u", 1:lags))

i_var <- indata_ts[, 3]  # Assuming the first column is the target variable
...
rf_model <- randomForest(y ~ ., data = rfdata_train, ntree = 1000, mtry=3)
Error.rate = rf_model$mse
plot(Error.rate, col="red")


# Further Tests on mtry uing Out of Bag Error. Lower number is better 
rf_model1 <- randomForest(y ~ ., data = rfdata_train, ntree = 200, mtry = 3)
...
for (i in 1:niter) {
  ...
  rf_model2 <- randomForest(y ~ ., data = rfdata_xtrain, ntree = 400, mtry = 4)
...
ggplot() +
  ...
  labs(title = "Comparison of Forecast Models for Median Home Prices", x = "Date", y = "Home Price") +
